...
@section('breadcrumb', 'Perbarui konten testimoni terpilih')
...
<div class="max-w-5xl mx-auto">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-brand-50 to-white">
            ...
            <div>
                <h2 class="font-semibold text-gray-800 text-sm">Form Edit Testimoni</h2>
                <p class="text-xs text-gray-400">Ubah data testimoni milik <strong>{{ $testimonial->name }}</strong></p>
            </div>
        </div>

        {{-- Form --}}
        <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" enctype="multipart/form-data" id="testimonial-form" class="p-6">
            @csrf
            @method('PUT')
